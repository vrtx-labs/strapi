        ...
        if (oldPublishedVersions.length > 0) {
            ...
            if (!params.data) {
                const transformData = (obj1)=>{
                    ...
                    let changes = {};
                    const staticAttributes = Object.values(contentTypes.constants);
            ...
            const updateParams = await async.pipe(validateParams, // sets query to filter for published or draft
            statusToLookupCurry(contentType), // sets publishedAt value
            statusToDataCurry(contentType), // Default locale will be set if not provided
            defaultLocaleCurry(contentType), localeToLookupCurry(contentType), localeToDataCurry(contentType))(params);
            // this would normally be handles by entries.publish so it is added here
            params.data['publishedAt'] = new Date();
            publishedEntries = await async.map(oldPublishedVersions, (published)=>entries.update(published, updateParams));
        } else publishedEntries = await async.map(updatedDraft, (draft)=>entries.publish(draft, queryParams));
